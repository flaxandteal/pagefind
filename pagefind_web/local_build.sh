...
if [ "$1" = "debug" ]; then
    ...
    RUSTUP_TOOLCHAIN=nightly wasm-pack build --release -t no-modules --manifest-path ./Cargo.toml -Z build-std=panic_abort,std
fi
node build.js
...
grep -e pagefind_stem/ Cargo.toml | while read line ; do
    if [ "$1" = "debug" ]; then
        ...
        RUSTUP_TOOLCHAIN=nightly wasm-pack build --release -t no-modules --features ${line:0:2} --manifest-path ./Cargo.toml -Z build-std=panic_abort,std
    fi
